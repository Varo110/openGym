...
describe('routine long-press reorder', () => {
  ...
  it('keeps the heading tappable and exposes a discoverable keyboard-focusable group cog', () => {
    mountRoutine([
      configured('1001', 'pair'), configured('1002', 'pair'), configured('1003'),
    ], { pair: { kind: 'complex', label: 'Clean + press' } })

    const heading = host.querySelector('button.ss-heading-button')
    const cog = host.querySelector('button[aria-label="Edit group"]')
    expect(heading?.textContent).toContain('Clean + press')
    expect(cog?.querySelector('svg')).toBeTruthy()
    expect(cog?.getAttribute('type')).toBe('button')
