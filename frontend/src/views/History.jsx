...
export default function History() {
  ...
  const workouts = S.workouts.filter(w => historyUnitCompatible(w, S.unit))
  return <>
    <div className="hdr"><button className="iconbtn" onClick={() => nav('/stats')} aria-label={t('Stats')}><Icon name="chevronLeft" /></button>
      <div style={{ flex: 1, marginLeft: 12 }}><h1>{t('History')}</h1><div className="sub">{t('{0} workouts', workouts.length)}</div></div></div>
    {workouts.length ? <div className="list">{[...workouts].reverse().map(w => <WorkoutRow key={w.id} w={w} onClick={() => workoutDetailSheet(w)} />)}</div>
