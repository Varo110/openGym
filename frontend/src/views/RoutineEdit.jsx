...
export default function RoutineEdit() {
  ...
  const edit = fn => update(s => { const routine = s.routines.find(x => x.id === id); if (routine) fn(routine.ex) })
  ...
  const move = (i, dir) => edit(ex => {
    ...
    if (ex[i]?.sg && ex[i].sg === ex[j]?.sg) {
      ;[ex[i], ex[j]] = [ex[j], ex[i]]
      return
    }
    const sourceUnit = supersetUnits(ex).findIndex(unit => unit.includes(i))
    if (sourceUnit < 0) return
    reorderRoutineUnit(ex, i, sourceUnit + (dir < 0 ? -1 : 1))
  })
  ...
  return <div className="narrow routine-editor">
    ...
    {r.ex.length ? <div ref={reorder.listRef} className={'list routine-list' + (reorder.drag ? ' is-reordering' : '')}>{r.ex.map((e, i) => {
      ...
      const occurrenceId = `${e.id}#${r.ex.slice(0, i).filter(candidate => String(candidate.id) === String(e.id)).length + 1}`
      ...
      return <div key={i} className={'routine-drag-row' + (isDragging ? ' is-dragging' : '')} data-routine-row data-ex-index={i}
        ...
        {unitFirst.has(i) && <div className="ss-label ss-heading" onClick={event => { event.stopPropagation(); groupMetaSheet(id, e.sg) }}>
          <button type="button" className="ss-heading-button" onClick={event => { event.stopPropagation(); groupMetaSheet(id, e.sg) }}>
            <Icon name="link" />{r.groupMeta?.[e.sg]?.label || t(r.groupMeta?.[e.sg]?.kind === 'complex' ? 'Complex' : 'Superset')}{r.groupMeta?.[e.sg]?.cues && <span className="small dim"> · {r.groupMeta[e.sg].cues}</span>}
          </button>
          <button type="button" className="iconbtn ss-heading-cog" aria-label={t('Edit group')} title={t('Edit group')} onClick={event => { event.stopPropagation(); groupMetaSheet(id, e.sg) }}><Icon name="gear" /></button>
        </div>}
        <div className={'item' + (inSS.has(i) ? ' in-ss' : '')} data-ex-index={i} data-ex-id={e.id}
          ...
          <div className="grow"><div className="tt capitalize">{ex.n}</div><div className="ss">{exLine(e, S.unit)}</div>
            <LastPerformance state={S} exerciseId={e.id} mode={modeOf(e)} occurrenceId={occurrenceId} />
            {e.notes && <div className="routine-note small dim">{e.notes}</div>}
          </div>
    ...
    <Button variant="primary" onClick={() => exercisePicker(ex => exConfigSheet(ex, null, cfg => edit(x => { x.push({ id: ex.id, ...cfg }) }), null, r, undefined, { useLastSetup: true, historyState: S }))} icon="plus">{t('Add exercise')}</Button>
    <div style={{ height: 10 }} />
