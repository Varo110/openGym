...
import { historyEntryCompatible, historyUnitCompatible, isWorkRow, modeForSet } from './workout-model.js'
...
export function derivePickupSuggestions({ workouts = [], exercises = [], unit = null, now = Date.now() } = {}) {
  ...
  for (const entry of exercises) {
    ...
    if (primary && finite(strength[primary])) suggestions[exercise.id] = Math.round(strength[primary] * 100)
  }
  return suggestions
}
