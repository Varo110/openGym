...
export function buildPlanBundle(S, name, { includePersonalCues = false } = {}) {
  ...
  const annotations = includePersonalCues
    ...
    : {}
  return {
    opengym_plan: PLAN_FMT, exported: todayISO(), name: name || '', unit: normalizeUnit(S.unit) || 'kg', week, routines, customEx,
...
export function parsePlan(raw) {
  ...
  const parsedUnit = data.unit == null || data.unit === '' ? null : planUnit(data.unit)
  if (data.unit != null && data.unit !== '' && !parsedUnit) {
