...
import { matchExercise, mergeImport, parseBodyweight, parseWorkoutCSV } from './import-csv.js'
...
describe('curated importer aliases', () => {
  ...
  it('maps generic and standing barbell press aliases to military press while retaining explicit seated presses', () => {
    const militaryAliases = [
      'overhead press', 'military press', 'shoulder press', 'ohp',
      'barbell overhead press', 'overhead press (barbell)', 'barbell ohp',
      'barbell military press', 'military press (barbell)',
      'barbell shoulder press', 'shoulder press (barbell)',
      'standing overhead press', 'standing military press', 'standing shoulder press', 'standing ohp',
      'barbell standing overhead press', 'standing barbell overhead press',
      'barbell standing military press', 'standing barbell military press',
      'barbell standing shoulder press', 'standing barbell shoulder press'
    ]
    const seatedAliases = [
      'seated overhead press', 'seated military press', 'seated shoulder press', 'seated ohp',
      'barbell seated overhead press', 'seated barbell overhead press', 'seated overhead press (barbell)',
      'barbell seated military press', 'seated barbell military press', 'seated military press (barbell)',
      'barbell seated shoulder press', 'seated barbell shoulder press', 'seated shoulder press (barbell)'
    ]
    for (const name of militaryAliases) expect(matchExercise(name), name).toBe('1457')
    for (const name of seatedAliases) expect(matchExercise(name), name).toBe('0091')
  })

  it('does not collapse specialized press names into military press', () => {
    const specialized = {
      'bradford press': '0105',
      'behind neck press': null,
      'smith shoulder press': '0766',
      'standing smith military press': '0774',
      'barbell standing close grip military press': '1456',
      'push press': '1700'
    }
    for (const [name, id] of Object.entries(specialized)) {
